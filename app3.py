...
# --- Query Input: text + audio ---
query = st.text_area("Ask a question across all data formats...")
query_audio = st.file_uploader("🎤 Or upload an audio question", type=["mp3", "wav"])

if query_audio:
    with st.spinner("🎧 Transcribing your question..."):
        query_audio_path = os.path.join("uploads", query_audio.name)
        os.makedirs("uploads", exist_ok=True)
        query_audio.seek(0)
        with open(query_audio_path, "wb") as f:
            f.write(query_audio.read())
        query = transcribe_audio(query_audio_path)
        st.success("Audio transcribed successfully!")
        st.write(f"**Transcribed Query:** {query}")

query_images = st.file_uploader("🖼️ Upload Images for Query", type=["png", "jpg", "jpeg"], accept_multiple_files=True)
...
if st.button("Ask"):
    ...
    for num, cid in citations:
        ...
        if doc_type == "audio":
            ...
            img_path = meta.get("image_name")
            if img_path and os.path.exists(img_path):
                st.image(img_path, width=300, caption=f"{pdf_name} - Page {page_no}", use_container_width=False)
        else:
            st.markdown(f"""
                <div style="border:1px solid #aaa; border-radius:6px; padding:8px; margin-bottom:8px;">
                    <div style="font-weight:bold; background-color:#e0e0e0; padding:4px 6px; border-radius:4px;">
