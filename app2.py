...
def remove_pdf_data(file_name):
    ...
    # Delete docs from ChromaDB
    if ids_to_delete:
        collection.delete(ids=ids_to_delete)

    # Remove PDF/audio file itself
...
def rerank(prompt, docs, ids):
    ...
    ranks = model.rank(prompt, docs, top_k=len(docs))  # <-- rank all docs
    final_text = ""
    citations = []
...
with st.sidebar:
    ...
    # Remove uploaded PDF + images + ChromaDB entries
    if remove_pdf_btn and uploaded_pdf:
        remove_pdf_data(uploaded_pdf.name)
        st.warning(f"Deleted {uploaded_pdf.name} and all related images/docs ✅")
